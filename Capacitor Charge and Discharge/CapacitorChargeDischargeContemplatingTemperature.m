% Clean screen, variables and figures
...
prompt = {'Voltaje (V)', 'Capacitancia (F)', 'Resistencia Carga (Ω)', 'Resistencia Descarga (Ω)', 'Tiempo de Almacenamiento (s)', 'Temperatura del Circuito (ºC)', 'Coeficiente de Temperatura (ºC^-1)'};
digtitle = 'Introduce los valores';
dims = [1 25];
definput = {'5','0.0002', '1000', '1000', '10', '0', '-0.0005'};
answer = inputdlg(prompt, digtitle, dims, definput);

% Convert the input strings to numbers
% Convertir lae entradas string a números
V = str2num(answer{1});
C = str2num(answer{2});
Rc0 = str2num(answer{3});
Rd0 = str2num(answer{4});
to = str2num(answer{5});
temperatura = str2num(answer{6});
a = str2num(answer{7});

% Calculate the values based on the input data
% Cálculo de diferentes valores a partir de los datos ingresados
Rc = Rc0 * (1 + (a * temperatura));       % Charge Resistance based on temperature     -  Resistencia de carga considerando la temperatura
Rd = Rd0 * (1 + (a * temperatura));       % Discharge REsistance based on temperature  -  Resistencia de descarga considerando la temperatura
tc = 5*Rc*C;                              % Charge time                                -  Tiempo de Carga
td = 5*Rd*C;                              % Discharge time                             -  Tiempo de Descarga
Q = C*V;                                  % Charge                                     -  Carga
U = (Q^2)/(2*C);                          % Stored Energy                              =  Energía acumulada
tt = tc+to+td;                            % Total time                                 =  Tiempo total
Pc = (V^2)/(Rc);                          % Maximum Charge Power                       =  Potencia máxima de carga
Pd = (V^2)/(Rd);                          % Macimum Discharge Power                    -  Potencia máxima de descarga
...
% Ask user if the current vs time graph is desired
...
switch respuesta_usuario
    case 'Yes'                                 % If answer is yes                                      -  Si la respuesta es si
        ...
        ti = [0:0.01:(tc+to)];                 % Create time vector from 0 to charge and storage time  -  Crear vector de tiempo de 0 a tiempo de carga y almacenamiento
        ...
        ti2 = ti1 + (tc+to);                   % Sum the charge and storage time to ti2 vector         -  Sumar el tiempo de carga y almacenamiento al vector ti2
        ...
        % Plot I and I2 using the respective time vectors ti and ti2
...
% Ask user if the charge vs time graph is desired
...
switch respuesta_usuario
    case 'Yes'                               % If answer is yes                                      -  Si la respuesta es si
        ...
        Q1 = (V*(1-exp(-(tq/(Rc*C))))) * C;  % Calaculate charge using tq vector                     -  Calcular carga respecto al vector de tq
        
        tq15 = [0:0.001:td];                 % Create time vector from 0 to discharge vector         -  Crear vector de tiempo de 0 a tiempo de descarga
        Q2 = (V * exp(-(tq15/(Rd*C)))) * C;  % Calaculate charge using tq15 vector                   -  Calcular carga respecto al vector de tq15
        tq2 = tq15 + (tc + to);              % Sum the charge and storage time to tq15 vector        -  Sumar el tiempo de carga y almacenamiento al vector tq15
        ...
        % Plot Q1 and Q2 using the respectives time vectors tq and tq2
...
fprintf('Resultados: \n')
fprintf(['  Resistencia de carga (Temperatura despreciable): %.2f Ω'], Rc0)
fprintf(['\n'])
fprintf(['  Resistencia de carga (Temperatura: %.2f ºC): %.2f Ω'], temperatura, Rc)
fprintf(['\n \n'])
fprintf(['  Resistencia de descarga (Temperatura despreciable): %.2f Ω'], Rd0)
fprintf(['\n'])
fprintf(['  Resistencia de descarga (Temperatura: %.2f ºC): %.2f Ω'], temperatura, Rd)
fprintf(['\n \n'])
